...
namespace DS
{
    public class ParticulasAguitaVersion3 : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if ((vector <= 140f) && (nivel > 0f))
            {
                ...
                if (FindObjectOfType<DrinkAlcoholBarLargoPlazo>().colisionVaso)
                {
            ...
            else
            {
                if (myParticleSystem.isPlaying && !IsFinished) //Si se acaba el agua o se deja de tomar agua
                {
                    ...
                    {
                        IsFinished = true;
                        FindObjectOfType<DrinkAlcoholBarLargoPlazo>().vasos++;
