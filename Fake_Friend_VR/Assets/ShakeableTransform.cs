...
public class ShakeableTransform : MonoBehaviour
{
    ...
    void Update()
    {
        transform.localPosition = new Vector3(Mathf.PerlinNoise(0, Time.time) * 2 - 1, 0, 0) * 0.3f;

        transform.localPosition = new Vector3(
            (Mathf.PerlinNoise(0, Time.time ) * 2 - 1)* 0.35f,
            (Mathf.PerlinNoise(1, Time.time ) * 2 - 1)*0.2f,
            (Mathf.PerlinNoise(2, Time.time ) * 2 - 1)*0.35f
        );
